DEFINE vhtmlpage='dbpie_result_cache01_'
DEFINE vtitlethispage='Result Cache'
DEFINE viconthispage='cache.gif'
...
SELECT ',[' ||''''|| name||'''' || ','||
       value|| ']'
from (
  SELECT  nvl(name, 'large_pool') name, round( sum(bytes)/1024/1024,2) value
  FROM V$SGASTAT
  where pool='shared pool' and name like 'Result%'
  GROUP BY name
)
/
PRO ]);
