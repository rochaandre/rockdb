DEFINE vhtmlpage='dbpie_index_unusable01_'
DEFINE vtitlethispage='Unusable index - internal schemas'
...
SELECT ',[' ||''''|| status||'''' || ','||
       total|| ']'
from (
SELECT 'UNUSABLE' status, count(*) total
from dba_indexes
...
group BY 'UNUSABLE'
UNION all
SELECT 'VALID' status, count(*) total
from dba_indexes
WHERE status<> 'UNUSABLE'
and owner in ('SYS','SYSTEM','ANONYMOUS','CTXSYS','HR','MDSYS','ODM','ODM_MTR','OE',
'OLAPSYS','ORDPLUGINS','ORDSYS','PM','QS','QS_ADM','QS_CB','QS_CBADM','QS_CS','QS_ES','QS_OS',
